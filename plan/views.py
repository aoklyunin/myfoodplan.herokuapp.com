...
from plan.forms import LoginForm, ProductSingleForm, AddProductForm, ProductForm, subdict
from plan.models import InfoText, Product
...
def detailProduct(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            d = subdict(form,
                        ('dimention', 'name', 'proteins', 'fats', 'carbohydrates', 'caloricity', 'cnt', 'dt', 'water'))
            Product.objects.filter(pk=product_id).update(**d)
            eq.tp = form.cleaned_data["tp"]
            eq.save()

    ef = ProductForm(instance=Product.objects.get(pk=product_id), prefix="main_form")
